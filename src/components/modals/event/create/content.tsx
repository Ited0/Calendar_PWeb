...
export const Content = () => {
  ...
  const { close, data } = useModal<SlotInfo>();
  ...
  async function onSubmit(values: z.infer<typeof CreateEventSchema>) {
    events.add(values as unknown as EventInfo).then(() => {
      enqueueSnackbar("Evento cadastrado", { 
        variant: "success"
       })
      close();
    }).catch((error) => {
      enqueueSnackbar("Erro ao Cadastrar: " + error.message, {
        variant: "error"
      })
    })
  }
